...
export const imageResolvers = {
  Mutation: {
    addEventImage: async (_: any, { eventId, url, alt }: { eventId: string; url: string; alt?: string }, context: Context) => {
      ...
      const canEdit = context.user!.roles.includes(Role.ADMIN) || 
    ...
    removeEventImage: async (_: any, { id }: { id: string }, context: Context) => {
      ...
      // Check if user can edit this event
      const canEdit = context.user!.roles.includes(Role.ADMIN) || 
                     image.event.organizerId === context.user!.id;
